...
public class ClassPathResource implements Comparable<ClassPathResource> {
	...
	public ClassPathResource(String rootPath, String fileNameWithAbsolutePath) {
		this(rootPath, fileNameWithAbsolutePath, Thread.currentThread().getContextClassLoader(),
				StandardCharsets.UTF_8);
	}

	public ClassPathResource(String rootPath, String fileNameWithAbsolutePath, ClassLoader classLoader,
			Charset encoding) {
		this.rootPath = rootPath;
		this.fileNameWithAbsolutePath = fileNameWithAbsolutePath;
		this.fileNameWithRelativePath = rootPath == null ? fileNameWithAbsolutePath
				: getPathRelativeToThis(fileNameWithAbsolutePath);
